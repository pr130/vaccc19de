...
test_that("Bundesland data cleaning works", {
  ...
  expect_equal(colnames(cleaned), c("bundesland", "bundesland_iso", "impfungen_kumulativ", "differenz_zum_vortag", "indikation_nach_alter", "berufliche_indikation", "medizinische_indikation", "pflegeheim_bewohner_in", "bundesland_notes"))
  expect_setequal(cleaned$bundesland, mapping$bundesland)
  expect_setequal(cleaned$bundesland_iso, mapping$bundesland_iso)
  expect_equal(dplyr::filter(cleaned, !is.na(.data$bundesland_notes)) %>% nrow, 3)
})
